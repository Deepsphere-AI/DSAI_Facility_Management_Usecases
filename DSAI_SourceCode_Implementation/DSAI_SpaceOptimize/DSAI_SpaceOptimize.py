...
def ReadFile():
    ...
    with col4:
        vAR_st.write('')
        vAR_st.write('')
        vAR_choice = vAR_st.selectbox("Select anyone",["Sensor Device","Surveillance Camera","Video Streaming","Database","File Upload"])
...
def ExtractInsightsWithLLM():
    ...
    if vAR_submit:   
        if vAR_user_input:
            vAR_llm_response = DataInsightsWithAssistant(vAR_user_input,ASSISTANT_ID)
